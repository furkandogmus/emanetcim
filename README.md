...
İş kuralları (max konaklama günü, sigorta, valiz çarpanları, iptal kesintisi vb.) `PlatformSettings` tablosunda tutulur; `prisma migrate` + `prisma db seed` ile `default` satırı oluşur.

CI: `main` dalına push / PR’da **migrate deploy**, **test** ve **build** (GitHub Actions, Postgres servis). ESLint uyarıları azalınca workflow’a `npm run lint` eklenebilir.
