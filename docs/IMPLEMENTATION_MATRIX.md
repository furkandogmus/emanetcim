...
| Checkout + iyzico split | Tam | Talep: `createBookingAction` (WAITING_APPROVAL). Ödeme: esnaf onayından sonra `/bookings/[id]/pay` → `payBookingWithIyzicoAction` → `initializeMarketplacePayment` + PAID; webhook idempotent |
