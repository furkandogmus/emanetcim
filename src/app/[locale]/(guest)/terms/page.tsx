...
/**
 * TODO (YAPILMADI -- bilinerek birakildi): asagidaki 14 bolumluk sozlesme metni
 * bu bilesende TURKCE SABIT duruyor (~60 paragraf). `Terms` ad alani alti dilde
 * de mevcut ama OLU: sayfa ondan yalnizca `lastUpdated`, `footerQuestion` ve
 * `footerContact` anahtarlarini okuyor; `q1..q6`/`a1..a6` ve `intro` hicbir
 * yerde cizilmiyor ve zaten bu sozlesmenin icerigiyle de ortusmuyor.
 *
 * SONUC: en/de/fr/fa/ja kullanicisi hukuken baglayici oldugu iddia edilen metni
 * TURKCE goruyor.
 *
 * NE YAPILMALI: 14 bolum `Terms.section1..section14` bicimiyle (baslik + paragraf
 * dizisi) locale dosyalarina tasinmali, ALTI dile de cevrilmeli ve asagidaki
 * `buildSections()` metni `getTranslations("Terms")` + `t.raw(...)` ile okumali.
 * Eskiyen `q1..q6`/`a1..a6`/`intro` anahtarlari ayni gecişte silinmeli.
 *
 * NEDEN SIMDI YAPILMADI: hukuki metnin makine cevirisi sozlesmenin anlamini
 * degistirir -- alti dilin de hukuki onaydan gecmesi gerekir. Ayrica ~60
 * paragrafin tasinmasi bu degisikligin kapsamini (sessiz Tailwind hatasi,
 * doldurulmamis yer tutucu, odeme metni celiskisi) tamamen asiyor.
 */

/**
 * Sozlesmenin TARAFI disaridan verilir.
 *
 * NEDEN: 1.1'de "[Sirket Tam Unvani]" yer tutucusu DOLDURULMAMIS halde yayinda
 * duruyordu; sayfa sitemap'te ve footer'dan linkli, yani baglayici oldugu one
 * surulen bir metnin taraflarindan biri belirsizdi. Tescilli unvan uydurulamaz,
 * bu yuzden deger ortam degiskeninden okunur ve verilmediginde o paragraf HIC
 * cizilmez: eksik bir taraf tanimi, YANLIS bir taraf tanimindan iyidir. Kalici
 * dogru ev `src/lib/config.ts` icinde `contact`/`branding` yanindaki bir alandir;
 * orasi bu degisikligin kapsami disinda oldugu icin degisken burada okunuyor.
 */
const COMPANY_LEGAL_NAME = process.env.NEXT_PUBLIC_COMPANY_LEGAL_NAME?.trim() || "";

if (!COMPANY_LEGAL_NAME && process.env.NODE_ENV !== "test") {
  console.warn(
    "[terms] NEXT_PUBLIC_COMPANY_LEGAL_NAME tanimsiz -- sozlesmenin 1.1 " +
      "(Sirket / Hizmet Saglayici) paragrafi cizilmiyor.",
  );
}
...
function buildSections(): Section[] {
  // Para cumlelerinin surumunu aktif odeme saglayicisi secer -- `payment-copy.ts`.
  const paymentMode = getPaymentCopyMode();

  return [
    {
      title: "1. TARAFLARIN TANIMLANMASI",
      paragraphs: [
        // Unvan verilmediginde bu paragraf hic cizilmez -- bkz. COMPANY_LEGAL_NAME.
        ...(COMPANY_LEGAL_NAME
          ? [
              `1.1. Şirket / Hizmet Sağlayıcı (BagajPark): İşbu sözleşmede; Ankara merkezli olarak faaliyet gösteren, bagaj depolama platformunun sahibi ve işletmecisi olan ${COMPANY_LEGAL_NAME} (Bundan sonra kısaca "Şirket" veya "Platform" olarak anılacaktır) ifade edilir.`,
            ]
          : []),
    ...
    },
    {
      ...
      paragraphs: [
        ...
        // 4.5 / 8.2 / 10.3: bu uc cumle "ödeme yalnızca uygulama üzerinden kart ile"
        // diyordu; urun su an DUKKANDA TAHSILAT ile calisiyor, yani sozlesme kullaniciyi
        // yapamayacagi bir sey icin taahhut altina sokuyordu. Cancellation/FAQ/Partners
        // sayfalarindaki cozumun aynisi: metnin surumunu saglayicinin yetenegi secer,
        // PSP baglandigi gun kartli surum kendiliginden geri gelir.
        paymentMode === "online"
          ...
          : "4.5. Rezervasyon ve Ödeme Doğruluğu: Kullanıcı, hizmet bedelini bagajı teslim ederken İş Ortağının noktasında ödeyeceğini; Platform üzerinden yapılan rezervasyonun bu ödeme yükümlülüğünü ortadan kaldırmadığını kabul eder.",
      ],
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        paymentMode === "online"
          ...
          : "8.2. Ödeme Yöntemleri ve Güvenlik: Hizmet bedeli, bagajın teslimi sırasında doğrudan İş Ortağının noktasında tahsil edilir. Platform şu aşamada kart ile online tahsilat yapmaz; bu nedenle kart bilgisi ne istenir ne de saklanır.",
    ...
    },
    {
      ...
      paragraphs: [
        ...
        paymentMode === "online"
          ...
          : "10.3. İade Yöntemi: Ödeme dükkanda tahsil edildiğinden, hizmet verilmeden iptal edilen rezervasyonlarda herhangi bir tahsilat yapılmaz; tahsil edilmiş bir bedelin iadesi ödemenin yapıldığı İş Ortağı noktasında gerçekleştirilir.",
