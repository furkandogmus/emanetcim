...
export async function generateMetadata({
  ...
  const { title, description } = getGuestStaticSeo(locale, "privacy");
  const base = getSiteBaseUrl();
  return {
    title,
    description,
    alternates: alternatesForPath(locale, "/privacy"),
    openGraph: { title, description, url: `${base}/${locale}/privacy` },
  };
}
    ...
    <div className="min-h-screen bg-gray-50/30">
      <header className="py-24 px-6 bg-white border-b border-gray-100 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center">
          <div className="w-20 h-20 bg-orange-50 rounded-4xl flex items-center justify-center text-orange-600 mx-auto mb-8 shadow-sm">
            <Lock size={36} />
          </div>
      ...
      <main className="max-w-4xl mx-auto py-24 px-6">
        <div className="space-y-12">
          ...
          {[1, 2, 3, 4, 5, 6].map((idx) => {
            ...
            const aKey = `a${idx}`;
            return (
              <section key={idx} className="group p-10 hover:bg-white rounded-4xl transition-all border border-transparent hover:border-gray-100 hover:shadow-xl hover:shadow-gray-200/40">
        ...
        <div className="mt-20 p-12 bg-gray-900 rounded-4xl text-center text-white">
          ...
          <a href="mailto:privacy@bagajpark.com" className="inline-flex h-14 items-center px-10 bg-orange-600 hover:bg-orange-700 text-white rounded-2xl id-eyebrow text-sm transition-all">
